...
void ModuleModelImport::LoadModel_Textured(ModuleGameObject* objMain, const char* meshPath, const char* texturePath)
{
	...
	if (scene != nullptr && scene->HasMeshes())
	{
		...
		for (uint i = 0; i < scene->mNumMeshes; i++)
		{
			...
			// bounding box thing
			//if (!meshComponent->bb_vertices.empty()) //Really need all this?
			//{
			...
			//meshComponent->bbox.Enclose((float3*)&meshComponent->mesh.num_vertices, (float)meshComponent->mesh.num_vertices / 3); // test causes crash at 40%
